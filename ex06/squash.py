...
class Ball:
    ...
    def __init__(self, fname, rack,mato,mato2):#初期化メソッド
        ...
        self.v_x = 2   #ボールの初期位置x座標を2に参照
        self.v_y = 2   #ボールの初期位置y座標を2に参照
        self.racket = rack
        self.mato = mato   #的１つ目参照
        self.mato2 = mato2 #的2つ目参照


    def ball_move(self, scr: Screen):
        global score # スコア表示の為の変数

        self.rct.centerx += int(self.v_x) #ボールのx座標の速度を更新
        self.rct.centery += int(self.v_y) #ボールのy座標の速度を更新

        if self.rct.left < scr.rct.left: #ボールが左の壁に反射したら
            self.rct.left = scr.rct.left
            self.v_x = -self.v_x
        if self.rct.right > scr.rct.right: #ボールが右の壁に反射したら
            self.rct.right = scr.rct.right
            self.v_x = -self.v_x
        if self.rct.top < scr.rct.top:      #ボールが上の壁に反射したら
...
def main(): #メイン関数
    ...
    rack = Racket("fig/bou3.png",0.08,(550,550)) #バーの設定
